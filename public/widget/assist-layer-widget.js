function AssistLayerWidget() {
    ...
    this.init = function (data) { //{app_id,identity_provider_url,botUserId}
        ...
        oReq.onload = function (e) {
            ...
            <iframe id="layer-widget" class="layer-widget slideInUp animated" src="https://assist-layer-widget.herokuapp.com/conversations/?app_id=`+ xhr.response.appId + `&identity_provider_url=` + data.identity_provider_url + `&chatBotCode=` + data.chatBotCode + `&environmentType=` + data.environmentType + `&botUserId=` + xhr.response.chatBotUserId + `">
            </iframe>
            `
        };
